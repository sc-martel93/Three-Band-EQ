    ...
    juce::Slider& slider)
{
    ...
    {
        // Rectangle for center of slider
        ...
        // Make sure stays in bounds of slider
                                    ...
                                    bool shouldDrawButtonAsDown)
{
    ...
    else if (auto* analyzerButton = dynamic_cast<AnalyzerButton*>(&toggleButton))
    {
        ...
        auto yVal = insetRect.getY() + insetRect.getHeight();
        
        randomPath.startNewSubPath(insetRect.getX(), yVal  * r.nextFloat());
        
        for (auto x = insetRect.getX() + 1; x < insetRect.getRight(); x += 2)
        {
            randomPath.lineTo(x, yVal * r.nextFloat());
        }
...
void RotarySliderWithLabels::paint(juce::Graphics& g)
{
    ...
    // 7 o'clock angle starting point for slider
    auto startAng = degreesToRadians(180.f + 45.f);

    // 5 o'clock angle for ending point of slider
   ...
   /* g.setColour(Colours::red);
    ...
    for (int i = 0; i < numChoices; ++i)
    {
        ...
        // Check that pos is within valid range
        jassert(0.f <= pos);
        jassert(pos <= 1.f);
...
juce::String RotarySliderWithLabels::getDisplayString() const
{
    ...
    if (auto* floatParam = dynamic_cast<juce::AudioParameterFloat*>(param))
    {
        ...
        // Set 2 decimal places if over 1000 (kHz)
...
{
    // Register as listener to params
...
void PathProducer::process(juce::Rectangle<float> fftBounds, double sampleRate)
{
    ...
    while (leftChannelFifo->getNumCompleteBuffersAvailable() > 0 )
    {
        ...
        {
            // Shift everything in monbuffer forward by however many samples incoming
            ...
            // Copy all samples from temp buffer to end of mono buffer
...
void ResponseCurveComponent::resized()
{
    ...
    // Render response grid labels
    g.setColour(Colours::lightgrey);
    const int fontHeight = 10;
    g.setFont(fontHeight);

    // Loop through freqs and label text above each line
    for (int i = 0; i < freqs.size(); ++i)
    {
        auto f = freqs[i];
        auto x = xs[i];
        
        // Add kHz label is needed
        bool addK = false;
        String str;
        if (f > 999.f)
        {
